...
void Stack<T>::push(const T& val) {
    if (is_full()) { throw std::runtime_error("Cannot push to full stack"); }
    _data.push_back(val);
}

template<class T>
void Stack<T>::pop() {
    if (is_empty()) { throw std::runtime_error("Cannot pop from empty stack"); }
    _data.pop_back();
}

template<class T>
inline T Stack<T>::top() const {
    if (is_empty()) { throw std::runtime_error("Cannot get top from empty stack"); }
    return _data.back();
}

...
// ввести функцию в эксплуатацию
int recalc_cell_num(int i, int j, int M) {
    return i* M + j;
}


std::pair<bool**, DSU<int>> generate(int X, int Y, int N, int M) {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            ...
            //int cur_cell = recalc_cell_num(i, j, M);

            //if ((random_on_the_right == 0) && (j != (M - 1)) && (maze.find_rec(cur_cell) != maze.find_rec(recalc_cell_num(i, j + 1, M)))) {   // + проверка на состояние в dsu, тогда не нужно убирать стену
            //    maze.union_sets(cur_cell, recalc_cell_num(i, j + 1, M));
            //    walls[i * 2 + 1][j + 1] = false;
            //}
            //if ((random_on_the_bottom == 0) && (i != (N - 1)) && (maze.find_rec(cur_cell) != maze.find_rec(recalc_cell_num(i + 1, j, M)))) {   // + проверка на состояние в dsu, тогда не нужно убирать стену
            //    maze.union_sets(cur_cell, recalc_cell_num(i + 1, j, M));
            //    walls[i * 2 + 2][j] = false;
            //}
    ...
    if (maze.find_rec(X) == maze.find_rec(Y)) return std::make_pair(walls, maze);
    else {
        for (int i = 0; i < new_N; i++) {
            delete[] walls[i];
        }
